...
def analyst_loop():
    ...
    while True:
        try:
            ...
            # 2H СКАН
            # ══════════════════════════════════════════════════
            # СЖАТИЕ 2H: топ-450 активных монет, без фильтра объёма.
            # Расширили с 350 → 450 чтобы захватывать монеты в зрелом сжатии,
            # у которых объём 24h естественно проседает (ZAMA-кейс: $370K на MEXC,
            # на 380-420 позиции по объёму). На MEXC всего ~500 пар USDT-perp,
            # топ-450 даёт почти полный охват.
            #
            # Внимание: на низколиквидных монетах ($5-50K) ATR Map даёт ложные
            # срабатывания. Если будет много спама — вернуть фильтр объёма.
            top450_2h = [x['s'] for x in vol_data[:450]]

            for symbol in top450_2h:
                try:
            ...
            logging.info(f"Итерация. Символов 4H: {len(symbols)} | 2H: {len(top450_2h)} | "
